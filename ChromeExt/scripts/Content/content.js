...
chrome.runtime.onMessage.addListener(async function (
  ...
  if (request.method === "get-highlighted-word") {
    ...
    if (highlighted && highlighted.length > 0) {
      ...
      let highlighted_word = window.getSelection().getRangeAt(0); // Get index of highlighted word
      let highlighted_start_index = highlighted_word.startOffset;

      let first_half_of_sentence;
      let second_half_of_sentence;

      let inputString;

      if (
        parent_element.textContent.lastIndexOf(".", highlighted_start_index) ==
        -1
      ) {
        inputString = parent_element.textContent;
        console.log(inputString);
      } else {
        first_half_of_sentence =
          parent_element.textContent.lastIndexOf(".", highlighted_start_index) +
          2; // Index of first letter in
        if (
          parent_element.textContent.indexOf(".", highlighted_start_index) == -1
        ) {
          inputString = parent_element.textContent;
          console.log(parent_element.textContent);
        } else {
          second_half_of_sentence = parent_element.textContent.indexOf(
            ".",
            highlighted_start_index
          ); // Index of last letter in sentence
          let surrounding_sentence = parent_element.textContent.substr(
            first_half_of_sentence,
            second_half_of_sentence
          );
          inputString = surrounding_sentence;
          console.log(surrounding_sentence);
        }
      }
      let response = {
        status: "OK",
        text: highlighted,
        parent_element_text: inputString,
      };
  ...
  if (request.method === "openPopup") {
    ...
    function createModal() {
      ...
      document
        .getElementById("wordBay-example-container")
        .appendChild(ul_example);
      document
        .getElementById("wordBay-synonyms-container")
        .appendChild(ul_synonym);
